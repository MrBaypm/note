...
| 能否关闭倒灌 | 需自定义包装类（实现复杂） | 本身不支持关闭，需切换为 SharedFlow | 是（replay=0 原生无倒灌） |
| 适用场景 | 简单状态数据管理 | 页面状态数据（需要持续持有最新值） | 一次性事件（弹窗、页面跳转、Toast） |

总结
StateFlow 默认有数据倒灌：它和 LiveData 一样会保存最新值，新观察者会立即收到，这是设计特性而非 bug，适合保存页面 “状态数据”。
解决倒灌的核心方案：
一次性事件（弹窗、Toast、跳转）：用 SharedFlow(replay=0)，彻底避免倒灌；
状态数据（页面展示的信息）：保留 StateFlow 的倒灌特性，或通过 “消费标记” 控制只处理一次。
关键原则：区分 “状态数据”（需要倒灌，如用户信息、列表数据）和 “一次性事件”（不需要倒灌，如弹窗），分别用 StateFlow 和 SharedFlow 处理。



最优替代方案：现代 Android 项目优先使用 StateFlow（状态数据）+ SharedFlow（一次性事件），解决 LiveData 所有痛点，且与协程深度集成，是官方推荐的主流方案；复杂异步场景可考虑 RxJava，简单跨组件通信谨慎使用 EventBus。
